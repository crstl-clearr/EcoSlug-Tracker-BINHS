...
class LogManager {
        ...
        try {
            ...
            this.unsubscribe = onSnapshot(q, (querySnapshot) => {
                ...
                querySnapshot.forEach((doc) => {
                    this.logEntries.push({ id: doc.id, ...doc.data() });
                });
                this.notifyListeners();
            });
        } catch (error) {
    ...
    exportToText() {
        ...
        exportText += `${'Entry#'.padEnd(8)} | ${'Date'.padEnd(12)} | ${'Time'.padEnd(8)} | ${'Plant/Area'.padEnd(20)} | ${'Type'.padEnd(15)} | ${'Dosage'.padEnd(20)} | Notes\n`;
         exportText += `${'-'.repeat(8)} | ${'-'.repeat(12)} | ${'-'.repeat(8)} | ${'-'.repeat(20)} | ${'-'.repeat(15)} | ${'-'.repeat(20)} | ${'-'.repeat(30)}\n`;
...
function toggleEditMode() {
    ...
    if (logManager.isEditMode) {
        ...
        // Scroll table to show actions column on mobile
        const tableWrapper = document.querySelector('.table-wrapper');
        if (tableWrapper) {
            tableWrapper.scrollLeft = tableWrapper.scrollWidth;
        }
